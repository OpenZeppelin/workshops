...
- `yarn deploy`: Compiles and deploys the Registry and Forwarder contracts to Sepolia, and writes their addresses in `deploy.json`.
...
- `yarn events`: Lists all the `Registered` events from the deployed contract on Sepolia.
...
- `REACT_APP_QUICKNODE_URL`: Optional URL to Quicknode for connecting to the Sepolia network from the dapp.
...
Create a relayer using [Defender Relay Client](https://docs.openzeppelin.com/defender/relay-api-reference) on Sepolia.
...
Use the newly created Relayer to deploy the ERC2771Forwarder and Registry contracts to Sepolia
...
We can then use the script to send the request to our relayer, and [view the transaction on Etherscan](https://sepolia.etherscan.io/).  We can also view the name registrations.
...
Create an [Autotask using Defender Client](https://docs.openzeppelin.com/defender/autotasks-api-reference), with a webhook trigger and connected to our Sepolia relayer.
